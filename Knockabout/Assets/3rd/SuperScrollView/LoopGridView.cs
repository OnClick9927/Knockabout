...
namespace SuperScrollView
{
    ...
    public class LoopGridView : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
    {
        ...
        //如果GridFixedType是GridFixedType。ColumnCountFixed，那么GridItemGroup是GridView的一行
        //如果GridFixedType是GridFixedType。RowCountFixed，那么GridItemGroup是GridView的一列
        //因此mItemGroupList是当前所有显示的行或列
        ...
        /*
         方法是初始化LoopGridView组件。有4个参数:
        itemTotalCount: GridView中的项目总数，该参数必须设置值>=0，则ItemIndex可以从0到itemTotalCount -1。
        onGetItemByRowColumn:当一个项目进入ScrollRect视口时，这个Action将被调用，项目索引和行、列索引作为数，让你创建项目和更新它的内容。
        setingparam:你可以使用这个参数来覆盖检查器设置中的值
        */
        ...
        /*
        此方法可用于在运行时设置GridView的项目总数。
        该参数的取值必须>=0,ItemIndex取值范围为0 ~ itemCount -1。
        如果resetPos设置为false，则在此方法完成后，ScrollRect的内容位置不会改变。
        */
        ...
        */
        ...
        public void SetItemPadding(Vector2 newPadding)
        {
            ...
            ForceUpdateItemPadding(newPadding);
        }

        public void ForceUpdateItemPadding(Vector2 newPadding)
        {
        ...
        public void ForceUpdatePadding(RectOffset newPadding)
        {
            mPadding = newPadding;
            UpdateStartEndPadding();
            UpdateContentSize();
