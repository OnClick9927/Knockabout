...
namespace Lockstep  {
    /// <summary>
    /// 二维整数网格坐标，接口与 Unity Vector2Int 接近，但不依赖 UnityEngine。
    /// 适合离散格子索引、寻路节点和不需要小数精度的确定性数据。
    /// </summary>
    public struct LVector2Int : IEquatable<LVector2Int> {
        ...
        public LFloat magnitude {
            get { return CalculateMagnitude(this.x, this.y); }
        }
        ...
        public static LFloat Distance(LVector2Int a, LVector2Int b){
            return CalculateMagnitude((long) a.x - b.x, (long) a.y - b.y);
        }

        private static LFloat CalculateMagnitude(long x, long y){
            x = x < 0 ? -x : x;
            y = y < 0 ? -y : y;
            long max = x > y ? x : y;
            if (max == 0) {
            ...
            return LMath.Sqrt(normalizedX * normalizedX + normalizedY * normalizedY) * max;
        }
