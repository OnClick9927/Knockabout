// Copyright 2019 谭杰鹏. All Rights Reserved //https://github.com/JiepengTan 

using System;

namespace Lockstep
{
    /// <summary>
    /// 3x3 定点矩阵，采用列优先存储，与 LQuaternion 的旋转矩阵转换配套使用。
    /// 一维索引按 row + column * 3 映射；构造函数的三个向量分别代表三列。
    /// </summary>
    public struct LMatrix33 : IEquatable<LMatrix33>
    {
        public static readonly LMatrix33
            zero = new LMatrix33(LVector3.zero, LVector3.zero, LVector3.zero);

        public static readonly LMatrix33 identity = new LMatrix33(LVector3.CreateFromRaw(LFloat.Precision, 0, 0),
            LVector3.CreateFromRaw(0, LFloat.Precision, 0), LVector3.CreateFromRaw(0, 0, LFloat.Precision));

        // mRowCol  列优先存储
        public long m00;
        public long m10;
        public long m20;
        public long m01;
        public long m11;
        public long m21;
        public long m02;
        public long m12;
        public long m22;

        public LMatrix33(LVector3 column0, LVector3 column1, LVector3 column2)
        {
            this.m00 = column0._x;
            this.m01 = column1._x;
            this.m02 = column2._x;
            this.m10 = column0._y;
            this.m11 = column1._y;
            this.m12 = column2._y;
            this.m20 = column0._z;
            this.m21 = column1._z;
            this.m22 = column2._z;
        }


        public LFloat this[int row, int column]
        {
            get { return this[row + column * 3]; }
            set { this[row + column * 3] = value; }
        }

        public LFloat this[int index]
        {
            get
            {
                switch (index)
                {
                    case 0:
                        return LFloat.FromRaw(  this.m00);
                    case 1:
                        return LFloat.FromRaw(  this.m10);
                    case 2:
                        return LFloat.FromRaw(  this.m20);
                    case 3:
                        return LFloat.FromRaw(  this.m01);
                    case 4:
                        return LFloat.FromRaw(  this.m11);
                    case 5:
                        return LFloat.FromRaw(  this.m21);
                    case 6:
                        return LFloat.FromRaw(  this.m02);
                    case 7:
                        return LFloat.FromRaw(  this.m12);
                    case 8:
                        return LFloat.FromRaw(  this.m22);
                    default:
                        throw new IndexOutOfRangeException("Invalid matrix index!");
                }
            }
            set
            {
                switch (index)
                {
                    case 0:
                        this.m00 = value._val;
                        break;
                    case 1:
                        this.m10 = value._val;
                        break;
                    case 2:
                        this.m20 = value._val;
                        break;
                    case 3:
                        this.m01 = value._val;
                        break;
                    case 4:
                        this.m11 = value._val;
                        break;
                    case 5:
                        this.m21 = value._val;
                        break;
                    case 6:
                        this.m02 = value._val;
                        break;
                    case 7:
                        this.m12 = value._val;
                        break;
                    case 8:
                        this.m22 = value._val;
                        break;
                    default:
                        throw new IndexOutOfRangeException("Invalid matrix index!");
                }
            }
        }

        public override int GetHashCode()
        {
            return this.GetColumn(0).GetHashCode() ^ this.GetColumn(1).GetHashCode() << 2 ^
                   this.GetColumn(2).GetHashCode() >> 2;
        }

        public override bool Equals(object other)
        {
            if (!(other is LMatrix33))
                return false;
            return this.Equals((LMatrix33) other);
        }

        public bool Equals(LMatrix33 other)
        {
            return this.GetColumn(0).Equals(other.GetColumn(0))
                   && this.GetColumn(1).Equals(other.GetColumn(1))
                   && this.GetColumn(2).Equals(other.GetColumn(2));
        }

        public static LMatrix33 operator *(LMatrix33 lhs, LMatrix33 rhs)
        {
            LMatrix33 mat;
            mat.m00 = (long) (((long) lhs.m00 * (long) rhs.m00 + (long) lhs.m01 * (long) rhs.m10 +
                           (long) lhs.m02 * (long) rhs.m20) / LFloat.Precision);
            mat.m01 = (long) (((long) lhs.m00 * (long) rhs.m01 + (long) lhs.m01 * (long) rhs.m11 +
                           (long) lhs.m02 * (long) rhs.m21) / LFloat.Precision);
            mat.m02 = (long) (((long) lhs.m00 * (long) rhs.m02 + (long) lhs.m01 * (long) rhs.m12 +
                           (long) lhs.m02 * (long) rhs.m22) / LFloat.Precision);
            mat.m10 = (long) (((long) lhs.m10 * (long) rhs.m00 + (long) lhs.m11 * (long) rhs.m10 +
                           (long) lhs.m12 * (long) rhs.m20) / LFloat.Precision);
            mat.m11 = (long) (((long) lhs.m10 * (long) rhs.m01 + (long) lhs.m11 * (long) rhs.m11 +
                           (long) lhs.m12 * (long) rhs.m21) / LFloat.Precision);
            mat.m12 = (long) (((long) lhs.m10 * (long) rhs.m02 + (long) lhs.m11 * (long) rhs.m12 +
                           (long) lhs.m12 * (long) rhs.m22) / LFloat.Precision);
            mat.m20 = (long) (((long) lhs.m20 * (long) rhs.m00 + (long) lhs.m21 * (long) rhs.m10 +
                           (long) lhs.m22 * (long) rhs.m20) / LFloat.Precision);
            mat.m21 = (long) (((long) lhs.m20 * (long) rhs.m01 + (long) lhs.m21 * (long) rhs.m11 +
                           (long) lhs.m22 * (long) rhs.m21) / LFloat.Precision);
            mat.m22 = (long) (((long) lhs.m20 * (long) rhs.m02 + (long) lhs.m21 * (long) rhs.m12 +
                              (long) lhs.m22 * (long) rhs.m22) / LFloat.Precision);
            return mat;
        }

        public static LVector3 operator *(LMatrix33 lhs, LVector3 vector3)
        {
            LVector3 vec;
            vec._x = ((long) lhs.m00 * vector3._x + (long) lhs.m01 * vector3._y +
                      (long) lhs.m02 * vector3._z) / LFloat.Precision;
            vec._y = ((long) lhs.m10 * vector3._x + (long) lhs.m11 * vector3._y +
                      (long) lhs.m12 * vector3._z) / LFloat.Precision;
            vec._z = ((long) lhs.m20 * vector3._x + (long) lhs.m21 * vector3._y +
                      (long) lhs.m22 * vector3._z) / LFloat.Precision;
            return vec;
        }

        public static bool operator ==(LMatrix33 lhs, LMatrix33 rhs)
        {
            return lhs.GetColumn(0) == rhs.GetColumn(0) && lhs.GetColumn(1) == rhs.GetColumn(1) &&
                   lhs.GetColumn(2) == rhs.GetColumn(2);
        }

        public static bool operator !=(LMatrix33 lhs, LMatrix33 rhs)
        {
            return !(lhs == rhs);
        }

        /// <summary>
        ///   <para>Get a column of the matrix.</para>
        /// </summary>
        /// <param name="index"></param>
        public LVector3 GetColumn(int index)
        {
            switch (index)
            {
                case 0:
                    return LVector3.CreateFromRaw(this.m00, this.m10, this.m20);
                case 1:
                    return LVector3.CreateFromRaw(this.m01, this.m11, this.m21);
                case 2:
                    return LVector3.CreateFromRaw(this.m02, this.m12, this.m22);
                default:
                    throw new IndexOutOfRangeException("Invalid column index!");
            }
        }

        /// <summary>
        ///   <para>Returns a row of the matrix.</para>
        /// </summary>
        /// <param name="index"></param>
        public LVector3 GetRow(int index)
        {
            switch (index)
            {
                case 0:
                    return LVector3.CreateFromRaw(this.m00, this.m01, this.m02);
                case 1:
                    return LVector3.CreateFromRaw(this.m10, this.m11, this.m12);
                case 2:
                    return LVector3.CreateFromRaw(this.m20, this.m21, this.m22);
                default:
                    throw new IndexOutOfRangeException("Invalid row index!");
            }
        }

        /// <summary>
        ///   <para>Sets a column of the matrix.</para>
        /// </summary>
        /// <param name="index"></param>
        /// <param name="column"></param>
        public void SetColumn(int index, LVector3 column)
        {
            this[0, index] = column.x;
            this[1, index] = column.y;
            this[2, index] = column.z;
        }

        /// <summary>
        ///   <para>Sets a row of the matrix.</para>
        /// </summary>
        /// <param name="index"></param>
        /// <param name="row"></param>
        public void SetRow(int index, LVector3 row)
        {
            this[index, 0] = row.x;
            this[index, 1] = row.y;
            this[index, 2] = row.z;
        }
    }
}
