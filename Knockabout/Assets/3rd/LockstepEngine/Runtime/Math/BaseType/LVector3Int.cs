...
namespace Lockstep {
    /// <summary>
    /// 三维整数网格坐标。适合体素、格点和离散空间索引；
    /// magnitude 等需要开方的结果仍使用 LFloat，以保持确定性和小数精度。
    /// </summary>
    public struct LVector3Int : IEquatable<LVector3Int> {
        /// <summary>为移植自 Unity 的向量实现提供最小数学函数集合。</summary>
        public class Mathf {
            /// <summary>
        ...
        public LFloat magnitude {
            get { return CalculateMagnitude(this.x, this.y, this.z); }
        }
        ...
        public static LFloat Distance(LVector3Int a, LVector3Int b){
            return CalculateMagnitude((long) a.x - b.x, (long) a.y - b.y, (long) a.z - b.z);
        }

        private static LFloat CalculateMagnitude(long x, long y, long z){
            x = x < 0 ? -x : x;
            y = y < 0 ? -y : y;
            z = z < 0 ? -z : z;
            long max = LMath.Max(x, LMath.Max(y, z));
            if (max == 0) {
                return LFloat.zero;
            }

            LFloat normalizedX = LFloat.FromRaw(x * LFloat.Precision / max);
            LFloat normalizedY = LFloat.FromRaw(y * LFloat.Precision / max);
            LFloat normalizedZ = LFloat.FromRaw(z * LFloat.Precision / max);
            return LMath.Sqrt(normalizedX * normalizedX + normalizedY * normalizedY + normalizedZ * normalizedZ) * max;
        }
