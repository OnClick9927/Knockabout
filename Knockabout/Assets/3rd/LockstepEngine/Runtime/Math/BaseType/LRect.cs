...
namespace Lockstep {
    /// <summary>
    /// 轴对齐的二维定点矩形，内部保存最小角 x/y 与最大角 xMax/yMax。
    /// 修改 position 会保持尺寸，修改 width/height 会固定最小角并移动最大角。
    /// 主要用于 2D 四叉树节点范围和包围盒查询。
    /// </summary>
    public struct LRect : System.IEquatable<LRect> {
        public LFloat x;
        public LFloat y;
        public LFloat xMax;
        public LFloat yMax;
        ...
        public LVector2 position {
            ...
            set {
                LFloat currentWidth = width;
                LFloat currentHeight = height;
                this.x = value.x;
                this.y = value.y;
                this.xMax = this.x + currentWidth;
                this.yMax = this.y + currentHeight;
            }
        }
        ...
        public override string ToString(){
            return
                $"(x:{(object) this.x:F2}, y:{(object) this.y:F2}, width:{(object) this.width:F2}, height:{(object) this.height:F2})";
        }
    }
}
