...
namespace Lockstep {
    /// <summary>
    /// 定点四元数，用于表达无万向节锁的三维旋转。
    /// Euler、Angle 和 eulerAngles 的公开角度单位为度，内部三角函数使用 LMath 查表。
    /// 参与空间变换前应保持单位长度；碰撞体入口会对外部四元数进行归一化保护。
    /// </summary>
    public struct LQuaternion : IEquatable<LQuaternion> {
        // 从旋转矩阵提取四元数或欧拉角时，用于循环取得另外两个轴。
        private static readonly int[] NextMatrixIndices = {1, 2, 0};
        ...
        /// <summary>用四个已经放大的原始定点值创建四元数。</summary>
        public static LQuaternion CreateFromRaw(long x, long y, long z, long w){
        ...
        public static LQuaternion identity {
            get { return CreateFromRaw(0, 0, 0, LFloat.Precision); }
        }
        ...
        public static LQuaternion AngleAxis(LFloat angle, LVector3 axis){
            if (axis.sqrMagnitude <= LFloat.EPSILON)
                return identity;
        ...
        public static LQuaternion FromToRotation(LVector3 fromDirection, LVector3 toDirection){
            if (fromDirection.sqrMagnitude <= LFloat.EPSILON ||
                toDirection.sqrMagnitude <= LFloat.EPSILON)
                return identity;

            LVector3 from = fromDirection.normalized;
            LVector3 to = toDirection.normalized;
            LFloat dot = LMath.Clamp(LVector3.Dot(from, to), LFloat.negOne, LFloat.one);
            if (dot >= LFloat.one - LFloat.EPSILON)
                return identity;

            if (dot <= LFloat.negOne + LFloat.EPSILON)
            {
                LVector3 axis = LVector3.Cross(LVector3.right, from);
                if (axis.sqrMagnitude <= LFloat.EPSILON)
                    axis = LVector3.Cross(LVector3.up, from);
                axis = axis.normalized;
                return new LQuaternion(axis.x, axis.y, axis.z, LFloat.zero);
            }

            LVector3 cross = LVector3.Cross(from, to);
            LQuaternion result = new LQuaternion(cross.x, cross.y, cross.z, LFloat.one + dot);
            LFloat norm = LMath.Sqrt(Dot(result, result));
            if (norm <= LFloat.EPSILON)
                return identity;
            return new LQuaternion(result.x / norm, result.y / norm, result.z / norm, result.w / norm);
        }
        ...
        public static LQuaternion Inverse(LQuaternion rotation){
            LFloat normSquared = Dot(rotation, rotation);
            if (normSquared <= LFloat.EPSILON)
                return identity;
            return new LQuaternion(
                -rotation.x / normSquared,
                -rotation.y / normSquared,
                -rotation.z / normSquared,
                rotation.w / normSquared);
        }
        ...
        public void ToAngleAxis(out LFloat angle, out LVector3 axis){
            LFloat clampedW = LMath.Clamp(w, LFloat.negOne, LFloat.one);
            angle = 2 * LMath.Acos(clampedW);
            LFloat axisLengthSquared = LFloat.one - clampedW * clampedW;
            if (axisLengthSquared <= LFloat.EPSILON) {
                axis = LVector3.right;
                angle = angle * 180 / LMath.PI;
                return;
            }

            LFloat div = 1 / LMath.Sqrt(axisLengthSquared);
        ...
        public override bool Equals(object other){
            return other is LQuaternion quaternion && Equals(quaternion);
        }

        public bool Equals(LQuaternion other) => this == other;
        ...
        private static LQuaternion MatrixToQuaternion(LMatrix33 m){
            ...
            if (fTrace > 0) {
                ...
                int j = NextMatrixIndices[i];
                int k = NextMatrixIndices[j];
